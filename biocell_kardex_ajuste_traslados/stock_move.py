...
from vst_kardex_fisico_valorado as vst_kardex_sunat
		...
		for xl in total_all:
			...
			if l['stock_moveid']:
				if mrp_flag:
					self.env.cr.execute(""" 
						...
						select sm.production_id,sm.id,sl_o.usage as origen , sl_d.usage as destino, sp.name,sm.origin, coalesce(sml.price_unit_it,0)*coalesce(sp.tc,1) as price_unit_it, sp2.name as name2, sp3.name as name3 from
						stock_move sm
						...
						where sm.id = """ +str(l['stock_moveid'])+ ( ("""  and sml.lot_id = """ + str(l['lot_id']) ) if l['lot_id'] else """ and sml.lot_id is null """ ) +  """
					 """)
					...
					self.env.cr.execute(""" 
						...
						select sml.id,sl_o.usage as origen , sl_d.usage as destino, sp.name,sm.origin, coalesce(sml.price_unit_it,0)*coalesce(sp.tc,1) as price_unit_it, sp2.name as name2, sp3.name as name3 from
						stock_move sm
						inner join stock_move_line sml on sml.move_id =  sm.id
						left join stock_picking sp on sp.id = sm.picking_id
						left join stock_picking sp2 on sp2.id = sp.backorder_id
					...
					if len(t) !=0:
						data_temp = t[0]
						if (data_temp['origen'] == 'inventory' or data_temp['origen'] == 'customer') and data_temp['price_unit_it'] == 0:								
							self.env.cr.execute(""" 		
								select coalesce(sml.price_unit_it,0)*coalesce(sp.tc,1) as price_unit_it from
								stock_move sm
								inner join stock_move_line sml on sml.move_id =  sm.id
								left join stock_picking sp on sp.id = sm.picking_id
								left join stock_picking sp2 on sp2.id = sp.backorder_id
								...
								inner join stock_location sl_d on sl_d.id = sm.location_dest_id
								where sm.id < """ +str(l['stock_moveid'])+ ( ("""  and sml.lot_id = """ + str(l['lot_id']) ) if l['lot_id'] else """ and sml.lot_id is null """ ) + """	
								and coalesce(sml.price_unit_it,0) != 0
								order by sm.kardex_date desc
								limit 1
							 """)	
							mg = self.env.cr.dictfetchall()
							if len(mg) != 0:
								self.env.cr.execute(""" 
							update stock_move_line set price_unit_it = """ + str(mg[0]['price_unit_it']) +"""  where move_id = """ +str(l['stock_moveid'])+ ( ("""  and lot_id = """ + str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ ) + """				
							 """)
								data_temp['price_unit_it'] = mg[0]['price_unit_it']
					else:			
						data_temp = {}
			...
			if l['stock_moveid'] and str(l['fecha'])[0:10]>= str(self.fecha_inicio)[0:10]:
				if data_temp['origen'] == 'internal' and data_temp['destino'] == 'internal' and l['salida']:
					self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( cprom_act_antes )+ """  where move_id = """ +str( l['stock_moveid'] )+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ ) + """; """)
				...
				if (data_temp['origen'] == 'internal' and data_temp['destino'] == 'transit') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'production') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'customer'):
					...
					self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin = 'Retorno de """ +str( data_temp['name'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					if data_temp['name2']:						
						...
						self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name2'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name2'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					if data_temp['name3']:						
						...
						self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name3'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name3'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					#self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( cprom_act_antes )+ """  where (origin_returned_move_id = """ +str( l['stock_moveid'] )+ """ ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
	...
	def do_valor_after(self):
		...
		self.env.cr.execute(""" 
			...
			select 
   				sml.id,
   				sl_o.usage as origen, 
       				sl_d.usage as destino,
	   			sp.name,
       				sm.origin, 
	   			coalesce(sml.price_unit_it,0)*coalesce(sp.tc,1) as price_unit_it, 
       				sml.product_id, 
	   			sml.lot_id
   			from
			stock_move sm
			...
			inner join stock_location sl_o on sl_o.id = sm.location_id
			inner join stock_location sl_d on sl_d.id = sm.location_dest_id
			where sm.kardex_date < '"""+str(fecha_arr)+"""'
		 """)

		for antiguos in self.env.cr.fetchall():
			self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it = """ +str( antiguos[5] )+ """  where picking_id in (select id from stock_picking where kardex_date >= '"""+str(fecha_arr)+"""' and  origin = 'Retorno de """ +str( antiguos[3] )+ """' ) and product_id = """ +str(antiguos[6])+ ((""" and lot_id = """ +str(antiguos[7]) ) if antiguos[7] else """ and lot_id is null """ )+ """; """)

		return {
			'type': 'ir.actions.client',
			'tag': 'notification_llikha',
			'params': {
				'title':'Actualización de Años Pasados',
				'type': 'success',
				'sticky': True,
		...
		for xl in total_all:
			...
			if l['stock_moveid'] and str(l['fecha'])[0:10]>= str(self.fecha_inicio)[0:10]:
				if data_temp['origen'] == 'supplier' and data_temp['destino'] == 'internal' and l['ingreso']:
					self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it_dolar = """ +str(( ((l['debit'] if l['debit'] else 0) +  (l['credit'] if l['credit'] else 0))/(l['ingreso']) ) if l['ingreso']!= 0 else 0 )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					
				if data_temp['origen'] == 'internal' and data_temp['destino'] == 'internal' and l['salida']:
					self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it_dolar = """ +str( cprom_act_antes )+ """  where id = """ +str( l['stock_moveid'] )+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
				...
				if (data_temp['origen'] == 'internal' and data_temp['destino'] == 'transit') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'production') or (data_temp['origen'] == 'internal' and data_temp['destino'] == 'customer'):
					...
					self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it_dolar = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					if data_temp['name2']:						
						...
						self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it_dolar = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name2'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name2'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					if data_temp['name3']:						
						...
						self.env.cr.execute(""" UPDATE stock_move_line set price_unit_it_dolar = """ +str( cprom_act_antes )+ """  where picking_id in (select id from stock_picking where origin like '%""" +str( data_temp['name3'] )+ """%' or origin = 'Retorno de """ +str( data_temp['name3'] )+ """' ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
					#self.env.cr.execute(""" UPDATE stock_move set price_unit_it_dolar = """ +str( cprom_act_antes )+ """  where (origin_returned_move_id = """ +str( l['stock_moveid'] )+ """ ) and product_id = """ +str(l['product_id'])+ ((""" and lot_id = """ +str(l['lot_id']) ) if l['lot_id'] else """ and lot_id is null """ )+ """; """)
