...
class AccountSunatRep(models.TransientModel):
	...
	def get_ventas_sire_1(self):
		...
		if self.type_show == 'pantalla':
			view = self.env.ref('biocell_integracion_sire.account_sunat_sire_sale_data_tree').id
			return {
	...
	def get_excel_sire(self,nro,type):
		...
		for l in BOOK_SIRE:
			if l["nro"] == nro and l["type"] == type:
				name_file = l["name"]
		workbook = ReportBase.get_excel_biocell_descarga_consultas_sql(sql,columns)
