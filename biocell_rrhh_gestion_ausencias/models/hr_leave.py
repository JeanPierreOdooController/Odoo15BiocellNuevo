...
class HrLeaveIt(models.Model):
    ...
    def _check_double_validation_rules(self, employees, state):
        if self.user_has_groups('biocell_rrhh_gestion_ausencias.group_hr_holidays_manager'):
            return
        is_leave_user = self.user_has_groups('biocell_rrhh_gestion_ausencias.group_hr_holidays_manager')
        if state == 'validate1':
    ...
    def write(self, values):
        is_officer = self.env.user.has_group('biocell_rrhh_gestion_ausencias.group_hr_holidays_manager') or self.env.is_superuser()
    ...
    def _unlink_if_correct_states(self):
        ...
        if not self.user_has_groups('biocell_rrhh_gestion_ausencias.group_hr_holidays_manager'):
    ...
    def _check_approval_update(self, state):
        ...
        if self.env.is_superuser():
            return
        current_employee = self.env.user.employee_id
        is_manager = self.env.user.has_group('biocell_rrhh_gestion_ausencias.group_hr_holidays_manager')
        ...
        for holiday in self:
            ...
            if holiday.state == 'draft':
                ...
                holiday.activity_schedule(
                    'biocell_rrhh_gestion_ausencias.mail_act_leave_approval',
                    note=note,
                    user_id=holiday.sudo()._get_responsible_for_approval().id or self.env.user.id)
            elif holiday.state == 'validate1':
                holiday.activity_feedback(['biocell_rrhh_gestion_ausencias.mail_act_leave_approval'])
                holiday.activity_schedule(
                    'biocell_rrhh_gestion_ausencias.mail_act_leave_second_approval',
                    note=note,
                    user_id=holiday.sudo()._get_responsible_for_approval().id or self.env.user.id)
                ...
                to_clean |= holiday
        if to_clean:
            to_clean.activity_unlink(['biocell_rrhh_gestion_ausencias.mail_act_leave_approval', 'biocell_rrhh_gestion_ausencias.mail_act_leave_second_approval'])
        if to_do:
            to_do.activity_feedback(['biocell_rrhh_gestion_ausencias.mail_act_leave_approval', 'biocell_rrhh_gestion_ausencias.mail_act_leave_second_approval'])
