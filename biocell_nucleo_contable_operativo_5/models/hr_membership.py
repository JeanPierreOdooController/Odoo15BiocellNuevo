...
class HrMembership(models.Model):
	...
	def get_membership_wizard(self):
		...
		return {
			...
			'views':[[self.env.ref('biocell_nucleo_contable_operativo_5.hr_membership_wizard_form').id,'form']],
	...
	def get_membership_wizard_edit(self):
		...
		return {
			...
			'view_mode':'form',
			'res_model':'hr.membership.wizard',
			'views':[[self.env.ref('biocell_nucleo_contable_operativo_5.hr_membership_wizard_form_edit').id,'form']],
			'context': self._context,
			'target':'new'
		}