...
class LogisticDespatch(models.Model):
    ...
    def action_despatch_sent(self):
        ...
        """
        self.ensure_one()
        template = self.env.ref('biocell_logistica_operativa.email_template_edi_despatch', raise_if_not_found=False)
        lang = get_lang(self.env)
        if template and template.lang:
            #lang = template._render_template(template.lang, 'biocell_logistica_operativa.despatch', [self.id])
            lang = template._render_lang(self.ids)[self.id]
        else:
            lang = lang.code
        compose_form = self.env.ref('biocell_logistica_operativa.logistic_despatch_send_wizard_form', raise_if_not_found=False)
        ctx = dict(
...
class LogisticExpedition(models.Model):
    ...
    def action_view_expedition_despatch(self):
        ...
        action = self.env.ref(
            'biocell_logistica_operativa.action_logistic_despatch_tree').read()[0]
        if len(res) > 0:
            action['domain'] = [
