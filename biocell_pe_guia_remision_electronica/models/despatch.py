...
class LogisticDespatch(models.Model):
    ...
    l10n_pe_dte_qr = fields.Char(string='QR Code', copy=False)
    ...
    bio_nota = fields.Text(string='Bio Nota')
    bio_vendedor = fields.Char(string='Bio Vendedor')
    bio_payment_term = fields.Char(string='Bio Payment Term')
    bio_nro_gaveta = fields.Char(string='Bio No Gaveta')
    bio_name_sale = fields.Char(string='Bio Name Sale')
    bio_valorizar = fields.Boolean(string='Bio Guia Valorizada')
    ...
    def _l10n_pe_prepare_dte(self):
        ...
        _despatch = {
            ...
            'field_3': self.bio_name_sale or '',
            'field_4': '1' if self.partner_id.precio_guia else '0',
            'field_5': self.bio_nota or '',
            'field_6': self.bio_vendedor or '',
            'field_7': self.bio_nro_gaveta or '',
            'field_8': self.bio_payment_term or '',
            'items': []
        }
        ...
        if self.line_ids:
            for line in self.line_ids:
                _item = {
                    ...
                    'field_1': line.bio_cantidad,
                    'field_2': line.bio_descuento,
                    'field_3': line.bio_pu,
                    'field_4': line.bio_total,
                    'field_5': line.product_id.categ_id.name,
                    'field_6': line.bio_lote,
                    'field_7': line.bio_fecven,
                }
...
class LogisticDespatchLine(models.Model):
    ...
    l10n_pe_dam_ds_code = fields.Char(string='Codigo DAM/DS', help="""Si el motivo de traslado es IMPORTACIÓN debe registrar el siguiente formato en el formulario del código DAM o DS.
        ...
        """)
    bio_cantidad = fields.Float(string='BIO Cantidad')
    bio_descuento = fields.Float(string='BIO Descuento')
    bio_pu = fields.Float(string='BIO PU')
    bio_total = fields.Float(string='BIO Total')
    bio_categoria = fields.Char(string='BIO Categoria')
    bio_lote = fields.Char(string='BIO Lote')
    bio_fecven = fields.Char(string='BIO Fec. Ven.')
    bio_package_id = fields.Many2one('stock.quant.package', string='Bio Package')
