odoo.define('web.SwitchCompanyBranchMenu', function(require) {
	...
	var SwitchCompanyBranchMenu = Widget.extend({
		...
		init: function(parent){
			this._super(parent);
			this.isMobile = false;
			this.biocell_gestion_sedes_empresas = [];
			this.current_biocell_gestion_sedes_empresa_id = 0;
			this.current_biocell_gestion_sedes_empresa_name = '';
		},
		start: function() {
			var self = this;
			...
			self._rpc({
				...
				if (self.isMobile) {
					...
					if(data.user_biocell_gestion_sedes_empresas){
						self.$('.oe_topbar_name').text(data.user_biocell_gestion_sedes_empresas.current_biocell_gestion_sedes_empresa[1]);    
					}
				}
				if(data.user_biocell_gestion_sedes_empresas){
					_.each(data.user_biocell_gestion_sedes_empresas.allowed_biocell_gestion_sedes_empresas, function(company) {
						var a = '';
						var b = '';
						var c = '';
						if (company[0] === data.user_biocell_gestion_sedes_empresas.current_biocell_gestion_sedes_empresa[0]) {
						...
						companies_list += '<div class="dropdown-item d-flex py-0 px-0" data-menu="biocell_gestion_sedes_empresa" data-company-branch-id="' + company[0] + '">' +
