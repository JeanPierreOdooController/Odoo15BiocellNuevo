...
class make_kardex_valorado(models.TransientModel):
	...
	clinic = fields.Boolean('No mostrar Clinicas',default=False)
	...
	def do_csvtoexcel(self):
		...
		import io
		#output = io.BytesIO()
		ruta_carpeta = "/home/odoo/reports/kardex_valorado.xlsx"
...
select 
			...
			left join stock_production_lot spt on spt.id = ksp.lote
			 ...
			 """+( (' and ksp.almacen != 39 ') if self.clinic else '' )
	...
	coalesce(coalesce(coalesce(po.name,so.name),sm.origin),sp.origin) as pedido
	...
	left join (
		select t_pp.id, 
	 	 ...
	 	 """ +( (' and sm.location_id != 39 and sm.location_dest_id != 39 ') if self.clinic else '' )+ """	
	...
	left join (
	 ...
	 """ +( (' and sm.location_id != 39 and sm.location_dest_id != 39 ') if self.clinic else '' )+ """	
		...
		for xl in total_all:
			...
			linea.append( str(move_obj.invoice_id.invoice_date) if move_obj.invoice_id.id else '' )
		...
		workbook.save(ruta_carpeta)
		workbook.close()
		with open(ruta_carpeta, "rb") as f:
			file_data = f.read()
		...
		for xl in total_all:
			...
			if str(self.fini) <= str(l['fechax'])[:10]:
				...
				self.env['guardado.kardex.company.line'].create({
					...
					'pedido_compra':'',
					'licitacion':str(move_obj.invoice_id.invoice_date) if move_obj.invoice_id.id and move_obj.invoice_id.invoice_date else '',
					'doc_almac':'',
...
class sql_kardex(models.Model):
	...
	def _get_function_get_kardex_v(self):#ESTO QUEDARA OBSOLETO                   select * from get_kardex_v(20220101,20231231,(select array_agg(id) from product_product),(select array_agg(id) from stock_location),2 );
		return """
...
almacen character varying,
 ...
 fecha_factura date
	...
	x.fecha_albaran,
	x.pedido_compra,
	''::character varying as licitacion,
	x.doc_almac,
	x.lote ,
	x.correlativovisual,
	 to_date(x.licitacion, 'YYYY-MM-DD') as fecha_factura 
				  from guardado_kardex_company_line as x
				  where x.location_id = ANY ($4) and x.product_id = ANY ($3)
      and x.product_id not in ( select xpp.id from product_product xpp inner join product_template xpt on xpt.id = xpp.product_tmpl_id where coalesce(xpt.no_accounting,false) = true )
	  and x.ubicacion_origen !=39 and x.ubicacion_destino !=39 
