...
class hr_contract(models.Model):
	...
	def export_contract(self):
		return self.env.ref('biocell_rrhh_documentos_contrato.action_report_contract_employee').report_action(self)

	def send_contract_email(self):
		for rec in self:
			template = self.env.ref('biocell_rrhh_documentos_contrato.report_contract_employee')
			email_values = {
