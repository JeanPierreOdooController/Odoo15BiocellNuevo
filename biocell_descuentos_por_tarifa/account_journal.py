	...
	def verify_lista_precio_tarifa(self):
		for i in self:
			if i.pricelist_id.id and i.pricelist_id.id != i.partner_id.property_product_pricelist.id:
				if not self.env.user.has_group("biocell_descuentos_por_tarifa.group_sale_order_tarifa_restriction"):
		...
		if "pricelist_id" in vals:
			for i in self:
				if i.pricelist_id.id != i.partner_id.property_product_pricelist.id:
					if not i.env.user.has_group("biocell_descuentos_por_tarifa.group_sale_order_tarifa_restriction"):
						raise UserError("Campo Lista de Precios solo puede ser modificada por el grupo 'Manejo Lista de Precio Venta' ")
		return t
...
class sale_order_line(models.Model):
	...
	def verify_discount_no_cero(self):
		for i in self:
			if i.discount != 0:
				if not self.env.user.has_group("biocell_descuentos_por_tarifa.group_sale_order_discount_restriction"):
