...
class SaleAdvancePaymentInv(models.TransientModel):
	...
	def _get_picking_domain(self):
		...
		res = {'domain': {'picking_ids': [('state', '=', 'done'), ('invoice_id', '=', False), ('id', 'in', sale.picking_ids.ids) , ('location_dest_id','=', self.env['stock.location'].search([('usage','=','customer')]).sorted(lambda r: r.id)[0].id  ) ]}}
		return res
