...
class make_kardex_valorado_stock(models.TransientModel):
	...
	def do_csvtoexcel(self):
		...
		linea = [
            border(ws,u"IDOdoo"),
            border(ws,u"Categoría de Producto N1"),
            border(ws,u"Categoría de Producto N2"),
            border(ws,u"Categoría de Producto N3"),
            border(ws,u"Categoría de Producto N4"),
            border(ws,u"Categoría de Producto N5"),
            border(ws,u"Marca"),
            border(ws,u"Codigo Producto"),
            border(ws,u"Producto"),
            border(ws,u"Unidad"),
            border(ws,u"Precio De Venta"),
            border(ws,u"Saldo Cantidad"),
            border(ws,u"Saldo Soles"),
            border(ws,u"C/U"),
            border(ws,u"Almacen"),
            border(ws,u"Lote"),
        ]
		...
		if self.moneda == 'pen':
			...
			self.env.cr.execute("""  
			 ...
			 from (
					...
					from (
                        ...
                        vst_kardex_sunat.fecha - interval '5' hour as fechax,sl_o.usage as origen_usage , sl_d.usage as destino_usage, np.new_name, spt.id AS lote
                        ...
                        left join stock_production_lot spt on spt.id = vst_kardex_sunat.lot_id
                    ...
                    ) A order by location_id,product_id,lote,fechax
			...
			self.env.cr.execute("""  
			 ...
			 from (
					...
					from (
                        select vst_kardex_sunat.*,sp.name as doc_almac,sm.kardex_date as fecha_albaran,
                        vst_kardex_sunat.fecha - interval '5' hour as fechax,sl_o.usage as origen_usage , sl_d.usage as destino_usage, np.new_name, spt.name AS lote
		...
		for xl in total_all:
			...
			if producto == None:
				...
				almacen = l['almacen']
				lote = l['lote']
			if producto != 	l['product_id'] or almacen != l['almacen'] or lote != l['lote']:
				producto = l['product_id']
				almacen = l['almacen']	
				lote = l['lote']
			...
			llave = (l['product_id'],l['location_id'],l['lote'])
			cprom_acum = [0,0]
			...
			loteobj = self.env['stock.production.lot'].browse(l['lote'] if l['lote'] else 0)
			linea.append( loteobj.name if loteobj.id else '' )
