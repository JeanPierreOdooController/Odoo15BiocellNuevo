...
class AccountInvoice(models.Model):
	...
	def action_open_asset_asset(self):
		self.ensure_one()
		action = self.env.ref('biocell_gestion_bienes_empresa_3.action_account_asset_asset_form').read()[0]
		domain = [('id', 'in', self.asset_asset_ids.ids)]
		context = dict(self.env.context, default_invoice_id=self.id)
		views = [(self.env.ref('biocell_gestion_bienes_empresa_3.view_account_asset_asset_purchase_tree').id, 'tree'), (False, 'form'), (False, 'kanban')]
