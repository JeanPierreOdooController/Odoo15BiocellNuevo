...
class product_pricelist_sale_import(models.TransientModel):
	...
	def verify_if_exists_product(self):
		...
		for row_no in range(sheet.nrows):
			...
			if row_no <= 0:
				...
				values.update( {'default_code': line[1],
							})
	...
	def tarifa_import(self):              
		...
		for row_no in range(sheet.nrows):
			...
			if row_no <= 0:
				...
				#percent_price
				tipo_linea = dic_casteotipo[line[0]]
				if tipo_linea == 'fixed':
					if self.import_tarifa_type == 'create':
						values.update( {'sku':line[1],
									'fixed_price': line[2],
									})
						...
						if tarif.id == False:
							...
							if producto:
								if len(producto)>1:
									raise UserError("Dos Productos con La Misma Referencia Interna: " + str(line[1]))							
								else:
								...
								raise UserError('Producto no Encontrado: ' + str(line[1]))
				else:
					if self.import_tarifa_type == 'create':
						values.update( {'sku':line[1],
									'percent_price': line[2],
									})
						res = self.product_create_porcentage(values)
					else:
						tarif = self.nombre_tarifa
						if tarif.id == False:
							raise UserError('Debe Escoger Una Tarifa')
							...
							if line[1].strip()=='':
								raise UserError('Campo REFERENCIA INTERNA no puede estar vacío')
							if line[2].strip() == '':
								raise UserError('Campo Descuento no puede estar vacío')
							producto = self.env['product.product'].sudo().search([('default_code', '=', line[1].strip()), '|', ('company_id', '=', self.env.company.id), ('company_id', '=', False)])
							if producto:
								if len(producto)>1:
									raise UserError("Dos Productos con La Misma Referencia Interna: " + str(line[1]))							
									...
									if len(line_tarif)==1:
										...
										if final[-1] == "'":
											final = final[:1]            
										line_tarif.sudo().write({'percent_price':float(final)})
							else:
								raise UserError('Producto no Encontrado: ' + str(line[1]))
	...
	def product_create_porcentage(self,values):		
		line_tarif = self.env['product.pricelist.item']
		if values.get('sku').strip() == '':
			raise UserError('Campo REFERENCIA INTERNA no puede estar vacío')			
		if values.get('percent_price').strip() == '':
			raise UserError('Campo DESCUENTO no puede estar vacío')
		tarif = self.nombre_tarifa
		if tarif.id == False:
			raise UserError('Debe Escoger Una Tarifa')
		...
		final = values.get('percent_price').strip()
		if final[0] == "'":
			final = final[1:]
		if final[-1] == "'":
			final = final[:1]
		...
		if line_tarif_repetida:
			if len(line_tarif_repetida)>1:
				...
				line_tarif_repetida.sudo().write({'percent_price':float(final)})
		else:
			vals = {
      				'pricelist_id': tarif.id,
					'applied_on': '0_product_variant',      
