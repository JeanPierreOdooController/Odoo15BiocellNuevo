...
class InvoiceApiController(Controller):
    @route(['/api/invoice'], type='http', auth="public", website=False, csrf=False,  methods=['GET'])
    def _get_product_data(self, **kwargs):
        ...
        try:
            ...
            required_token = request.env['powerbi.api.key'].sudo().search([
                ('name','=','invoice_api_powerbi')
            ])
            if token != required_token.token:
                raise ValidationError("Invalid Token")     
            query=''.join(request.env['invoice.powerbi.api'].get_sql())
