...
class TestExportSqlQuery(TransactionCase):
    def setUp(self):
        ...
        self.biocell_descarga_consultas_sql_obj = self.env["sql.export"]
        self.wizard_obj = self.env["sql.file.wizard"]
        self.sql_report_demo = self.env.ref("biocell_descarga_consultas_sql.biocell_descarga_consultas_sql_partner")

    def test_sql_query(self):
        wizard = self.wizard_obj.create(
            {
                "biocell_descarga_consultas_sql_id": self.sql_report_demo.id,
            }
        )
        wizard.export_sql()
    ...
    def test_prohibited_queries(self):
        prohibited_queries = [
            ...
            "DELETE FROM biocell_descarga_consultas_sql WHERE name = 'test';",
            "  DELETE FROM biocell_descarga_consultas_sql WHERE name = 'test'   ;",
            """DELETE
            FROM
            biocell_descarga_consultas_sql
            WHERE name = 'test'
            """,
        ]
        for query in prohibited_queries:
            with self.assertRaises(UserError):
                biocell_descarga_consultas_sql = self.biocell_descarga_consultas_sql_obj.create(
                    {"name": "test_prohibited", "query": query}
                )
                biocell_descarga_consultas_sql.button_validate_sql_expression()
    ...
    def test_authorized_queries(self):
        ...
        for query in authorized_queries:
            ...
            biocell_descarga_consultas_sql.button_validate_sql_expression()
            self.assertEqual(
                biocell_descarga_consultas_sql.state, "sql_valid", "%s is a valid request" % (query)
            )

    def test_sql_query_with_params(self):
        query = self.env.ref("biocell_descarga_consultas_sql.biocell_descarga_consultas_sql_partner_with_variables")
        categ_id = self.env.ref("base.res_partner_category_0").id
        wizard = self.wizard_obj.create(
            {
                "biocell_descarga_consultas_sql_id": query.id,
