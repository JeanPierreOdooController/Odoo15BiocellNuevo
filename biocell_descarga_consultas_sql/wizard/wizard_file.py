# Copyright (C) 2015 Akretion (<http://www.akretion.com>)
# @author: Florian da Costa
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl.html).

import json
from datetime import datetime

from lxml import etree

from odoo import api, fields, models
from odoo.tools import DEFAULT_SERVER_DATETIME_FORMAT
from odoo.exceptions import UserError


class SqlFileWizard(models.TransientModel):
    _name = "sql.file.wizard"
    _description = "Allow the user to save the file with sql request's data"

    binary_file = fields.Binary("File", readonly=True)
    file_name = fields.Char("File Name", readonly=True)
    biocell_descarga_consultas_sql_id = fields.Many2one(comodel_name="sql.export", required=True)

    @api.model
    def fields_view_get(
        self, view_id=None, view_type="form", toolbar=False, submenu=False
    ):
        """
        Display dynamically parameter fields depending on the biocell_descarga_consultas_sql.
        """
        res = super(SqlFileWizard, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu
        )
        export_obj = self.env["sql.export"]
        if view_type == "form":
            biocell_descarga_consultas_sql = export_obj.browse(self.env.context.get("active_id"))
            if biocell_descarga_consultas_sql.field_ids:
                eview = etree.fromstring(res["arch"])
                group = etree.Element("group", name="variables_group", colspan="4")
                toupdate_fields = []
                for field in biocell_descarga_consultas_sql.field_ids:
                    toupdate_fields.append(field.name)
                    attrib = {"name": field.name, "required": "0", "readonly": "0"}
                    view_field = etree.SubElement(group, "field", attrib=attrib)
                    modifiers = json.loads(view_field.get("modifiers", "{}"))
                    if field.required:
                        modifiers["required"] = True
                    view_field.set("modifiers", json.dumps(modifiers))

                res["fields"].update(self.fields_get(toupdate_fields))
                placeholder = eview.xpath(
                    "//separator[@string='variables_placeholder']"
                )[0]
                placeholder.getparent().replace(placeholder, group)
                res["arch"] = etree.tostring(eview, pretty_print=True)
        return res

    def export_sql(self):
        self.ensure_one()
        biocell_descarga_consultas_sql = self.biocell_descarga_consultas_sql_id

        # Manage Params
        variable_dict = {}
        if biocell_descarga_consultas_sql.field_ids:
            for field in biocell_descarga_consultas_sql.field_ids:
                if field.ttype == "many2one":
                    variable_dict[field.name] = self[field.name].id
                elif field.ttype == "many2many":
                    variable_dict[field.name] = tuple(self[field.name].ids)
                else:
                    variable_dict[field.name] = self[field.name]
        if "%(company_id)s" in biocell_descarga_consultas_sql.query:
            company_id = self.env.company.id
            variable_dict["company_id"] = company_id
        if "%(user_id)s" in biocell_descarga_consultas_sql.query:
            user_id = self.env.user.id
            variable_dict["user_id"] = user_id

        # Call different method depending on file_type since the logic will be
        # different
        method_name = "%s_get_data_from_query" % biocell_descarga_consultas_sql.file_format
        data = getattr(biocell_descarga_consultas_sql, method_name)(variable_dict)
        extension = biocell_descarga_consultas_sql._get_file_extension()
        self.write(
            {
                "binary_file": data,
                "file_name": "%(name)s.%(extension)s"
                % {"name": biocell_descarga_consultas_sql.name, "extension": extension},
            }
        )
        return {
            "view_mode": "form",
            "res_model": "sql.file.wizard",
            "res_id": self.id,
            "type": "ir.actions.act_window",
            "target": "new",
            "context": self.env.context,
            "nodestroy": True,
        }
    
    def button_preview_sql_expression(self):
        self.ensure_one()
        biocell_descarga_consultas_sql = self.biocell_descarga_consultas_sql_id
        # Manage Params
        variable_dict = {}
        if biocell_descarga_consultas_sql.field_ids:
            for field in biocell_descarga_consultas_sql.field_ids:
                if field.ttype == "many2one":
                    variable_dict[field.name] = self[field.name].id
                elif field.ttype == "many2many":
                    variable_dict[field.name] = tuple(self[field.name].ids)
                else:
                    variable_dict[field.name] = self[field.name]
        if "%(company_id)s" in biocell_descarga_consultas_sql.query:
            company_id = self.env.company.id
            variable_dict["company_id"] = company_id
        if "%(user_id)s" in biocell_descarga_consultas_sql.query:
            user_id = self.env.user.id
            variable_dict["user_id"] = user_id
        res = biocell_descarga_consultas_sql._execute_sql_request(params=variable_dict)
        raise UserError("\n".join(map(lambda x: str(x), res[:100])))