...
class make_kardex_valorado_formato_sunat(models.TransientModel):
	...
	def reporteador(self,obj_move):
		...
		for line in self.env.cr.fetchall():
			if line[21] != almacen_index or line[9] != product_index or line[26] != lote:
				primero = True
			...
			#lote_obj = self.env['stock.production.lot'].browse(lote)
			c.drawString( 216 ,pos_inicial,self.particionar_text( ( lote or '') + ( line[7] if line[7] else self.env.company.name_get()[0][1]) ,160) )
    ...
    lote as lot_id
		...
		primero = True
		for line in self.env.cr.fetchall():
			if line[21] != almacen_index or line[9] != product_index or line[25] != lote:
				primero = True
				if product_index != False:
				...
				linea = [border(ws,u"Fecha"),border(ws,u"Lote"),border(ws,u"Tipo"),border(ws,u"Serie"),border(ws,u"Numero"),border(ws,u"Tipo de operación"),border(ws,u"Cliente / Proveedor"),border(ws,u"Entrada Cantidad"),border(ws,u"Entrada C.U."),border(ws,u"Entrada Costo"),border(ws,u"Salida Cantidad"),border(ws,u"Salida C.U."),border(ws,u"Salida Costo"),border(ws,u"Saldo Final Cantidad"),border(ws,u"Saldo Final C.U."),border(ws,u"Saldo Final Costo")]
			...
			#lote_obj = self.env['stock.production.lot'].browse(lote)
			ws.append([str(line[1]) if line[1] else '', lote or '' ,str(line[2]) if line[2] else '',str(line[3]) if line[3] else '',str(line[4]) if line[4] else '',str(line[8]) if line[8] else '',str(line[7]) if line[7] else '','{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[11] if line[11] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[12] if line[12] else 0)/(line[11] if line[11] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[12] if line[12] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[13] if line[13] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[14] if line[14] else 0)/(line[13] if line[13] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[14] if line[14] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[15] if line[15] else 0) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % ( (line[16] if line[16] else 0)/(line[15] if line[15] else 1)     ) )),  '{:,.2f}'.format(decimal.Decimal ("%0.2f" % (line[16] if line[16] else 0) ))])
