...
class CheckingBalanceWizard(models.TransientModel):
	...
	def get_window_checking_register(self):
		if self.show_account_entries:
			view = self.env.ref('biocell_panel_saldos_financieros_3.view_checking_register_tree_true').id
		else:
			view = self.env.ref('biocell_panel_saldos_financieros_3.view_checking_register_tree').id
		return {
	...
	def get_window_checking_balance(self):
		view = self.env.ref('biocell_panel_saldos_financieros_3.view_checking_balance_tree').id
		return {
			'type': 'ir.actions.act_window',
