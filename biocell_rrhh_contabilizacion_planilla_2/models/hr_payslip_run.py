...
class HrPayslipRun(models.Model):
	...
	def get_sql(self):
		struct_id = self.env['hr.payroll.structure'].search(
			[('schedule_pay', '=', 'monthly'), ('active', '=', True), ('company_id', '=', self.env.company.id)],
			limit=1).id
		SalaryRules = self.env['hr.salary.rule'].search([('is_detail_cta', '=', True), ('struct_id', '=', struct_id)],
														order='sequence')
		if SalaryRules:
	...
	--    round(sum(prm.total * hadl.percent * 0.01)::numeric, 2) as debit,
	    ...
	    left join res_partner rp on he.address_home_id = rp.id
	    where prm.payslip_run_id = {payslip_run_id} and
	    prm.company_id = {company} and
	    prm.account_debit is not null
	    group by prm.sequence, prm.salary_rule_id,prm.description,aa.check_moorage, aaa.id, prm.account_debit, rp.id
	    ) lo
	    group by
	    lo.sequence,
	    lo.salary_rule_id,
	...
	--        round(total::numeric, 2) as credit,
	    ...
	    group by hc.membership_id,hm.name, hm.account_id, rp.id
						)T
