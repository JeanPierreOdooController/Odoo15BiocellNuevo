...
class MultipaymentAdvanceIt(models.Model):
	...
	@api.depends('company_id','journal_id','asiento_id','asiento_id.line_ids','company_id.account_journal_payment_debit_account_id','company_id.account_journal_payment_credit_account_id','journal_id.account_multipayment_id')
	def compute_amount_cash(self):
		for m in self:
			amount = 0
			if m.asiento_id:
				line_cash = m.lines_ids.filtered(lambda l: l.is_account_cash)
				for i in m.asiento_id.line_ids.filtered(lambda l: l.account_id.id == line_cash.account_id.id):
		...
		for line_i in self.invoice_ids:
			if self.journal_id.check_retention:
				if retention_precentage == 0:
					raise UserError(u'Debe configurar el porcentaje de PM en el Diario %s'%(self.journal_id.name))
	...
	def crear_asiento(self):
		...
		for i in self.lines_ids:
			vals = (0,0,{
					...
					'invoice_date_it':self.payment_date,
				})
			lineas.append(vals)
	...
	def biocell_carga_comprobantes_lines(self):
