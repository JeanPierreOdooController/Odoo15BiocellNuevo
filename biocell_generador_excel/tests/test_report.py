...
class TestReport(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xlsx_report = cls.env["report.biocell_generador_excel.abstract"].with_context(
            active_model="res.partner"
        )
        cls.report_name = "biocell_generador_excel.partner_xlsx"
    ...
    def test_currency_format(self):
        ...
        self.assertEqual(
            self.xlsx_report._biocell_generador_excel_currency_format(usd), "$#,##0.00"
        )
        eur = self.env.ref("base.EUR")
        self.assertEqual(
            self.xlsx_report._biocell_generador_excel_currency_format(eur), "#,##0.00 €"
        )
