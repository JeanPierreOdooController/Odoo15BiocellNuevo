...
class LandedCostIt(models.Model):
	...
	picking_units_ids = fields.Many2many('stock.picking', 'gastos_vinculado_picking_units_rel', 'gastos_id', 'picking_id', string='Albaranes')
	detalle_ids = fields.One2many('landed.units.specific.line', 'gastos_id', 'Detalle')
	company_id = fields.Many2one('res.company',string=u'Compañia',required=True, default=lambda self: self.env.company,readonly=True)
	invoice_ids = fields.One2many('landed.units.invoice.line', 'landed_id',string='Facturas')
	purchase_ids = fields.One2many('landed.units.purchase.line', 'landed_id',string='Ordenes de Compra')
	advalorem_ids = fields.One2many('landed.units.advalorem.line', 'landed_id',string='Advalorem')
	...
	def get_info(self):
		...
		picks = self.env['stock.picking'].search([('landed_units_id','=',self.id)])
		
		if picks:
			for p in picks:
				self.picking_units_ids = [(6, 0, [p.id])]
		
		self.agregar_lineas()
		invoices = self.env['account.move'].search([('landed_units_id','=',self.id),('state','=','posted')])
	...
	def get_invoices(self):
		wizard = self.env['get.landed.units.invoices.wizard'].create({
			'landed_id': self.id,
			'company_id':self.company_id.id
		})
		module = __name__.split('addons.')[1].split('.')[0]
		view = self.env.ref('%s.view_get_landed_units_invoices_wizard' % module)
		return {
			'name':u'Seleccionar Facturas',
			'res_id':wizard.id,
			'view_mode': 'form',
			'res_model': 'get.landed.units.invoices.wizard',
			'view_id': view.id,
	...
	def get_purchases(self):
		wizard = self.env['get.landed.units.purchases.wizard'].create({
			'landed_id': self.id,
			'company_id':self.company_id.id
		})
		module = __name__.split('addons.')[1].split('.')[0]
		view = self.env.ref('%s.view_get_landed_units_purchases_wizard' % module)
		return {
			'name':u'Seleccionar Compras',
			'res_id':wizard.id,
			'view_mode': 'form',
			'res_model': 'get.landed.units.purchases.wizard',
			'view_id': view.id,
			'context': self.env.context,
			'target': 'new',
			'type': 'ir.actions.act_window',
	...
	def unlink(self):
		...
		for i in self.picking_units_ids:
			i.unlink()
	...
	def borrador(self):
		self.state = 'draft'
		for i in self.detalle_ids:
			for l in i.stock_move_id.move_line_ids:
				if l.lot_id.id:
					l.lot_id.gasto_vinculado = 0
					l.lot_id.precio_final = l.lot_id.precio_compra + l.lot_id.gasto_vinculado
		#for i in self.detalle_ids:
		#	costo_actual = 0
		#	cantidad_actual = 0
	...
	def procesar(self):
		...
		for i in self.detalle_ids:
			for l in i.stock_move_id.move_line_ids:
				total = i.flete / i.stock_move_id.product_qty
				if l.lot_id.id:
					l.lot_id.gasto_vinculado = total
					l.lot_id.precio_final = l.lot_id.precio_compra + l.lot_id.gasto_vinculado
	...
	def agregar_lineas(self):
		...
		for i in self.picking_units_ids:
			for j in i.move_lines: 
				...
				self.env['landed.units.specific.line'].create(data)
	...
	def get_excel_saldos(self):
		...
		sql3 = """ valormn """
		for elem in self.env['landed.units.specific.type'].search([]):
			sql2 += "+"
			sql3 += "+"
			...
			sql += """ a.factor*(select sum(debit) from landed_units_invoice_line where landed_id=%d and type_landed_cost_id = %d) as "%s" """%(self.id,elem.id,elem.code)
		...
		%s
								...
								from biocell_importaciones_costos_adicionales_2_line a
								LEFT JOIN 
								(
								...
								from biocell_importaciones_costos_adicionales_2_line line
								...
								LEFT JOIN (select landed_id, picking_id , product_id, sum(coalesce(valormn,0)) as valormn from landed_units_advalorem_line GROUP BY landed_id, picking_id, product_id) c ON c.product_id = b.product_id AND a.gastos_id = c.landed_id and SM.picking_id = c.picking_id
								where a.gastos_id = %d
								)tt)
								UNION ALL
...
class LandedCostItLine(models.Model):
	_name = 'landed.units.specific.line'
	stock_move_id = fields.Many2one('stock.move', 'Stock Move')
	gastos_id = fields.Many2one('landed.units.specific', 'Gastos Vinculado',ondelete="cascade")
...
class LandedCostInvoiceLine(models.Model):
	_name = 'landed.units.invoice.line'
	
	landed_id = fields.Many2one('landed.units.specific', 'Gastos Vinculado',ondelete="cascade")
	...
	type_landed_cost_id = fields.Many2one('landed.units.specific.type',string='Tipo G.V.')
	company_id = fields.Many2one('res.company',string=u'Compañía')

class LandedCostPurchaseLine(models.Model):
	_name = 'landed.units.purchase.line'
...
class LandedCostAdvaloremLine(models.Model):
	_name = 'landed.units.advalorem.line'
	_description = 'Landed units Advalorem Line'
	...
	landed_id = fields.Many2one('landed.units.specific', 'Gastos Vinculado')
	invoice_id = fields.Many2one('account.move.line',string='Factura')
