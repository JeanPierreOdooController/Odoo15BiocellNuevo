...
class GetLandedInvoices(models.TransientModel):
	_name = "get.landed.units.invoices.wizard"
	
	landed_id = fields.Many2one('landed.units.specific',string='Gasto Vinculado')
	...
	def insert(self):
		...
		for invoice in self.invoices_ids:
			invoice_exist = self.env['landed.units.invoice.line'].search([('invoice_id','=',invoice.id)],limit=1)
			if invoice_exist:
			...
			val = {
				...
				'tc': invoice.tc,
				'type_landed_cost_id': invoice.product_id.type_landed_units_id.id,
				'company_id': invoice.company_id.id,
			}
			vals.append(val)
		self.env['landed.units.invoice.line'].create(vals)
