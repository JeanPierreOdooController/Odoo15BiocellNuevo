...
class AccountOpeningIt(models.Model):
	...
	def preview_opening(self):
		...
		self.env.cr.execute("""select a.account_id,c.currency_id,
							case when sum(coalesce((a.debit-a.credit),0))> 0 then sum(coalesce((a.debit-a.credit),0)) else 0 end as activo,
							case when sum(coalesce((a.debit-a.credit),0))< 0 then abs(sum(coalesce((a.debit-a.credit),0))) else 0 end as pasivo,
							...
							having sum(coalesce((a.debit-a.credit),0)) <> 0"""%(self.from_fiscal_year_id.date_from.strftime('%Y/%m/%d'),
		...
		for account in accounts:
			...
			a.nro_comp as nro_comprobante,b.currency_id,sum(coalesce((a.debit-a.credit),0)) as saldo_mn,
			sum(coalesce(a.amount_currency,0)) as saldo_me, min(a.date) as fecha_con, max(a.date_maturity) as fecha_ven
			from account_move_line a
			...
			having sum(coalesce((a.debit-a.credit),0))<>0)T
	...
	def generate_opening(self):
		...
		for account in accounts:
			...
			self.env.cr.execute("""select a.partner_id,a.account_id,a.type_document_id,a.nro_comp as nro_comprobante,b.currency_id,sum(coalesce((a.debit-a.credit),0)) as saldo_mn,
