...
class AccountBookPurchaseWizard(models.TransientModel):
	...
	def get_report(self):
		...
		if self.type_show == 'pantalla':
			...
			if self.currency == 'pen':
				view_id = self.env.ref('biocell_registro_adquisiciones.view_account_book_purchase_view_tree')
			else:
				view_id = self.env.ref('biocell_registro_adquisiciones.view_account_book_purchase_usd_view_tree')
			return {
	...
	def getCsv(self):
		ReportBase = self.env['report.base']
		workbook = ReportBase.get_file_biocell_descarga_consultas_sql(self._get_sql(),',',True)
