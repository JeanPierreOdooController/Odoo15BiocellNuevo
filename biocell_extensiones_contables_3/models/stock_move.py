		...
		return """
			...
			CREATE OR REPLACE VIEW public.vst_kardex_fisico1_lote AS 
			 SELECT stock_move.product_uom,
					CASE
						WHEN sl.usage::text = 'supplier'::text THEN sml.price_unit_it::double precision
						ELSE
						CASE
							WHEN uom_uom.id <> uomt.id THEN round((sml.price_unit_it::double precision * uomt.factor::double precision / uom_uom.factor::double precision)::numeric, 6)::double precision
							ELSE sml.price_unit_it::double precision
						END
					END AS price_unit,
					CASE
			...
			CREATE OR REPLACE VIEW public.vst_kardex_fisico1_lote_dolar AS 
			 SELECT stock_move.product_uom,
					CASE
						...
						(
							CASE WHEN l_o.usage = 'supplier' and coalesce(stock_picking.tc,1) > 1 then sml.price_unit_it::double precision
							WHEN l_o.usage = 'supplier' and coalesce(stock_picking.tc,1) = 1 then sml.price_unit_it::double precision / coalesce(rcr_e.sale_type,1)
							WHEN l_o.usage in ('inventory','production') then sml.price_unit_it::double precision / coalesce(rcr_e.sale_type,1)
							else sml.price_unit_it_dolar::double precision end
			...
			sm.product_uom,
					CASE
						WHEN uom_uom.id <> uomt.id THEN round((
						(
							CASE WHEN sls.usage = 'supplier' then sml.price_unit_it::double precision
							WHEN sls.usage in ('inventory','production') then sml.price_unit_it::double precision / coalesce(rcr_e.sale_type,1)
							else sml.price_unit_it_dolar::double precision end
						 ) * uomt.factor::double precision / uom_uom.factor::double precision)::numeric, 6)::double precision
						ELSE (
							CASE WHEN sls.usage = 'supplier' then sml.price_unit_it::double precision
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
   ...
   FROM biocell_importaciones_costos_adicionales gvd
	 JOIN biocell_importaciones_costos_adicionales_line gvdd ON gvdd.gastos_id = gvd.id
  ...
  WHERE gvd.state::text = 'done'::text and coalesce(pt.check_unidad_especifica,false) = false
 ...
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
	((gvdd.flete + gvdd.advalorem)/totalsm.total)*sml.qty_done AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
    ...
    FROM biocell_importaciones_costos_adicionales gvd
	 ...
	 JOIN stock_move sm ON gvdd.stock_move_id = sm.id
	 join (
	 		select sum(sml_x.qty_done) as total,
	 		sm_x.id
	 ...
	 left join account_analytic_account aaait on aaait.id = sm.analytic_account_id
	 LEFT JOIN ir_translation it ON pt.id = it.res_id AND it.name::text = 'product.template,name'::text AND it.lang::text = 'es_PE'::text AND it.state::text = 'translated'::text
  WHERE gvd.state::text = 'done'::text and coalesce(pt.check_unidad_especifica,false) = true;
...
CREATE OR REPLACE VIEW public.vst_kardex_fisico_gastos_vinculados_dolar AS 
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
	(gvdd.flete + gvdd.advalorem) / coalesce(rcr_e.sale_type,1) AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
...
CASE WHEN COALESCE(mpit.check_gastos_vinculados,false) then sm.kardex_date + interval '1 second' else
 ...
 SELECT pt.uom_id AS product_uom,
	NULL::integer AS move_dest_id,
	( ((gvdd.flete + gvdd.advalorem)/totalsm.total)*sml.qty_done / coalesce(rcr_e.sale_type,1) ) AS price_unit,
	0 AS product_qty,
	NULL::integer AS location_id,
	sl.id AS location_dest_id,
	sp.picking_type_id,
