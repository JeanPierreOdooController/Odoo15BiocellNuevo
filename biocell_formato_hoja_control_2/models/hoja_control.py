...
def format_cel_calibri_11_numberf(ws, texto):
    cell = WriteOnlyCell(ws, value=texto)
    cell.font = Font(name='Calibri', size=11, bold=False)
    cell.alignment = Alignment(horizontal='right', vertical='center')
    # Configura los bordes de la celda
...
class biocell_formato_pedido_venta_report_hj_xlsx(models.AbstractModel):
    _inherit = 'report.biocell_fhc.biocell_formato_hoja_control_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, so):
        ...
        ws.append([(''),atencion,(''),font_calibri_11(ws,': '+str(so.medic_center if so.medic_center else '')),(''),font_calibri_11_bold_upper(ws,'ESTADO'),font_calibri_11(ws,': '+ ( 'APROBADO' if so.aprob_id else 'DESAPROBADO' ))])

       
       
        ws.append([""])
        ws.append([(''),format_cel_calibri_11_bold_centered(ws,'CÓDIGO'),format_cel_calibri_11_bold_centered(ws,'DESCRIPCIÓN'),format_cel_calibri_11_bold_centered(ws,'LOTE'),format_cel_calibri_11_bold_centered(ws,'CANTIDAD'),format_cel_calibri_11_bold_centered(ws,'P. UNIT.'),format_cel_calibri_11_bold_centered(ws,'P. TOTAL.')])
        subtotal=0
        for move in so.move_ids_return:
            for line in move.move_line_ids:
                if line.qty_consumida > 0:
                    ws.append([(''),format_cel_calibri_11_centered_1(ws,str(line.product_id.default_code) if line.product_id.default_code else " "),format_cel_calibri_11_number(ws, str(line.product_id.name)),format_cel_calibri_11_centered_1(ws,str(line.lot_id.name or '')),format_cel_calibri_11_centered_1(ws,str(int(line.qty_consumida))),format_cel_calibri_11_number(ws,"S/ " + str('{0:,.2f}'.format(move.price_unit_sale))),format_cel_calibri_11_number(ws,"S/ " + str('{0:,.2f}'.format(line.qty_consumida * move.price_unit_sale)))])
                    subtotal = subtotal + (line.qty_consumida * move.price_unit_sale)
        
        ws.append([""])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"SUBTOTAL"),format_cel_calibri_11_number(ws,"S/ " + str('{0:,.2f}'.format(subtotal)))])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"IGV"),format_cel_calibri_11_number(ws,"S/ " + str('{0:,.2f}'.format(subtotal*0.18)))])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"TOTAL"),format_cel_calibri_11_number(ws,"S/ " + str('{0:,.2f}'.format(subtotal+(subtotal*0.18))))])
...
class SaleOrder(models.Model):
    ...
    def descargar_hoja_control(self):
        ...
        atencion = font_calibri_11_bold_upper(ws,'CENTRO MÉDICO')
        ws.append([(''),atencion,(''),font_calibri_11(ws,': '+str(self.medic_center if self.medic_center else '')),(''),font_calibri_11_bold_upper(ws,'ESTADO'),font_calibri_11(ws,': '+ ( 'APROBADO' if so.user_confirmación else 'DESAPROBADO'))])
        ...
        for move in self.move_ids_return:
            for line in move.move_line_ids:
                if line.qty_consumida > 0:
                    ws.append([(''),format_cel_calibri_11_centered_1(ws,str(line.product_id.default_code) if line.product_id.default_code else " "),format_cel_calibri_11_number(ws, str(line.product_id.name)),format_cel_calibri_11_centered_1(ws,str(line.lot_id.name or '')),format_cel_calibri_11_centered_1(ws,str(int(line.qty_consumida))),format_cel_calibri_11_numberf(ws,"S/ " + str('{0:,.2f}'.format(move.price_unit_sale))),format_cel_calibri_11_numberf(ws,"S/ " + str('{0:,.2f}'.format(line.qty_consumida * move.price_unit_sale)))])
                    subtotal = subtotal + (line.qty_consumida * move.price_unit_sale)
        ws.append([""])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"SUBTOTAL"),format_cel_calibri_11_numberf(ws,"S/ " + str('{0:,.2f}'.format(subtotal)))])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"IGV"),format_cel_calibri_11_numberf(ws,"S/s " + ('{0:,.2f}'.format(subtotal*0.18)))])
        ws.append([(''),(''),(''),(''),(''),format_cel_calibri_11_bold_centered(ws,"TOTAL"),format_cel_calibri_11_numberf(ws,"S/ " + str( '{0:,.2f}'.format(subtotal+(subtotal*0.18))))])
