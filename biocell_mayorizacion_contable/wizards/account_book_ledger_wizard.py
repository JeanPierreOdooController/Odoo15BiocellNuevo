...
class AccountBookLedgerWizard(models.TransientModel):
	...
	def _get_sql(self):
		...
		sql = """SELECT
			...
			REGEXP_REPLACE(may.glosa, '[^a-zA-Z0-9-]', '', 'g') AS glosa,
			may.td_partner,may.doc_partner,may.partner,
	...
	def get_excel(self):
		...
		workbook = ReportBase.get_excel_biocell_descarga_consultas_sql(self._get_sql(),self.get_header())
