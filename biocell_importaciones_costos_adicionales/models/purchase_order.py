...
class PurchaseOrder(models.Model):
	...
	def create_landed_cost(self):
		...
		return {
			'view_mode': 'form',
			'view_id': self.env.ref('biocell_importaciones_costos_adicionales.view_biocell_importaciones_costos_adicionales_form').id,
	...
	def show_landed_cost(self):
		self.ensure_one()
		action = self.env.ref('biocell_importaciones_costos_adicionales.action_biocell_importaciones_costos_adicionales').read()[0]
		domain = [('id', 'in', [self.landed_cost_id.id])]
		context = dict(self.env.context, default_invoice_id=self.id)
		views = [(self.env.ref('biocell_importaciones_costos_adicionales.view_biocell_importaciones_costos_adicionales_tree').id, 'tree'), (False, 'form'), (False, 'kanban')]
