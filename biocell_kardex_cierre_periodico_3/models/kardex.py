...
class biocell_generador_periodos_kardex(models.TransientModel):
...
#	def unlink(self):
#		if self.env.user.has_group('biocell_kardex_cierre_periodico_3.group_generator'):
#			for i in self:
...
class KardexSave(models.Model):
	...
	def aprobar_costo_cero(self):
		if self.env.user.has_group('biocell_kardex_cierre_periodico_3.group_aprob_costo_cero'):
			fecha = datetime.now()
			for i in self:
	...
	def aprobar_oper_sunat(self):
		if self.env.user.has_group('biocell_kardex_cierre_periodico_3.group_aprob_sunat_oper'):
...
#biocell_extensiones_contables_3