...
class AccountJournal(models.Model):
	...
	def generar_wizard(self):
		...
		return {
			...
			'views':[[self.env.ref('biocell_numerador_asientos.sequence_wizard_view').id,'form']],
	...
	def action_create_sequence(self):
		...
		return {
			...
			'views':[[self.env.ref('biocell_numerador_asientos.view_account_sequence_journal_wizard_form').id,'form']],
			'type':'ir.actions.act_window',
			'target':'new'
		}