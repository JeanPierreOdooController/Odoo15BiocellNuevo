...
class stock_move_line(models.Model):
	...
	def edit_kardex_date(self):
		return {			
			...
			'view_id': self.env.ref("biocell_kardex_fabricacion_2.move_line_fecha_kardex").id,
			'target': 'new',
	...
	def edit_mostrar_no(self):
		return {			
			...
			'view_mode': 'form',
			'res_model': 'stock.move.line',
			'view_id': self.env.ref("biocell_kardex_fabricacion_2.move_line_no_mostrar").id,
			'target': 'new',
			'type': 'ir.actions.act_window',
		}
...
class MrpProduction(models.Model):
	...
	def button_mark_done(self):
		...
		for line in self.move_raw_ids:
			if line.quantity_done > line.forecast_availability:
