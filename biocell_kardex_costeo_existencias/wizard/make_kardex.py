...
class stock_move(models.Model):
		...
		for self in selfs:
			if 'nomore' in self.env.context:
				...
				if self.purchase_line_id.id and 'price_unit_it' not in vals and not self.origin_returned_move_id.id:
					...
					self.move_line_ids.with_context({'nomore':1}).write({'price_unit_it':elem,'price_unit_it_dolar':elem})
					...
					self.move_line_ids.with_context({'nomore':1}).write({'price_unit_it':self.origin_returned_move_id.price_unit_it,'price_unit_it_dolar':self.origin_returned_move_id.price_unit_it_dolar})
		return t
	...
	def actualizar_priceunit(self):
		return {
			...
			'target': 'new',
			'res_id': self.id,
			'views': [(self.env.ref('biocell_kardex_costeo_existencias.stockmove_editpriceunit').id, 'form')],
		}
