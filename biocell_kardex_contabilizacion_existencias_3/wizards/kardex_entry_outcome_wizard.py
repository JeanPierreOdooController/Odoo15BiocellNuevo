...
class KardexEntryOutcomeWizard(models.TransientModel):
	...
	def get_report(self):
		...
		self.env.cr.execute("""
		INSERT INTO kardex_entry_outcome_book (fecha,tipo,serie,numero,doc_almacen,ruc,empresa,tipo_op,tipo_name, producto, default_code, unidad, qty, amount, cta_debe, cta_haber, origen, destino, almacen, analytic_account_id,analytic_tag_id,lot) 
	...
	def get_excel(self):
		...
		'CTA DEBE','CTA HABER',u'UBICACIÓN ORIGEN',u'UBICACIÓN DESTINO',u'ALMACÉN',u'CTA ANALÍTICA','ETIQUETA ANALITICA','LOTE']
		...
		for line in dic:
			...
			worksheet.write(x,20,line.lot if line.lot else '',formats['especial1'])
			x += 1

		widths = [10,6,7,9,16,12,48,8,41,20,9,11,14,15,15,33,33,11,22,20,10]
	...
	def _get_sql(self,date_ini,date_end,company_id):
		...
		if not param.out_operation_type_ids:
			raise UserError(u'Falta configurar Parámetro de "Tipo de Operación Salidas" en Parametros de Contabilidad de la Compañía.')
		sql_out_type_operation = "AND ei12.id in (%s)"%(','.join("%d"%i.id for i in param.out_operation_type_ids))
		...
		sql = """SELECT T.* FROM(SELECT 
				...
				WHERE (GKV.fecha::date BETWEEN '{date_ini}' AND '{date_end}') and coalesce(GKV.credit,0) > 0
				{sql_out_type_operation})T
		...
		""".format(
				...
				sql_out_type_operation = sql_out_type_operation
			)
		return sql
